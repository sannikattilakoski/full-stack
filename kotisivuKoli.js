...
function getWeatherData () {
        ...
        fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=63.09674&lon=29.80618&exclude=hourly,minutely&lang=fi&units=metric&appid=${api_key}`)
        .then(res => res.json())
        .then(data =>{
    ...
    moment.locale('fi'); // viikonpäivät tulevat suomeksi
...
function createPalauteListItem(palaute) {
    ...
    let date = document.createTextNode(palaute.date) 
     // lisätään tekstit p, par ja div2 elementteihin
    p.appendChild(text)
